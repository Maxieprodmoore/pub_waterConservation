...
for(var i = 0, len = elms.length; i < len; i++)new Splide(elms[i]).mount();
document.addEventListener('DOMContentLoaded', function() {
    elms.mount();
});
/*--------Setting up Slider for Uses of Water --------*/ var slideWaterUses = new Splide('#slide_waterUses', {
    type: 'fade',
    perPage: 1,
    padding: 0,
    rewind: true,
...
/*--------Setting up Slider for Water Available --------*/ var slidesWaterAvail = new Splide('#slider-waterAvail', {
    type: 'fade',
    perPage: 1,
    padding: 0,
    rewind: true,
...
/*Water Available Section----------*/ /*Get Water Trend button ID----*/ let waterTrendBtn = document.getElementById('waterTrend-btn');
/*Get Breakdown of water usage button ID----*/ let waterUsageBtn = document.getElementById('waterUsage-btn');
/*Get Water Trend image ID----*/ let waterTrendImg = document.getElementById('waterTrends');
/*Get Breakdown of water usage Image ID----*/ let waterUsageImg = document.getElementById('waterUsage');
/*Create Activator where when Water Trend button is clicked, water trend image is displayed.*/ waterTrendBtn.addEventListener("click", function() {
    waterTrendImg.style.display = "inline-block";
    document.getElementById("waterNeeds").style.display = "block";
});
/*Create Activator where when Water Usage button is clicked, water Usage image is displayed.*/ waterUsageBtn.addEventListener("click", function() {
    waterUsageImg.style.display = "inline-block";
});
/*-------Checking Watersupplies---------*/ /* Get Number slot-----*/ var waterAvailNum = document.getElementById('waterAvailPercent');
//let waterAvailNum = 0;
/*Get Rainfall Checkbox-----*/ const rain = document.getElementById('rainfall');
...
//};
//});
/*Getting Quiz Sheet------*/ var quizForm = document.getElementById('qnForm');
/*Getting the Quiz Questions------*/ var question = document.getElementById('qn');
var quizSet = document.querySelectorAll('.qnOptions');
var choiceA = document.getElementById('choiceA');
var choiceB = document.getElementById('choiceB');
var choiceC = document.getElementById('choiceC');
var choiceD = document.getElementById('choiceD');
/*Getting the Submit Answer Btn------*/ var ansBtn = document.getElementById('ansSub');
/*Getting the Quiz Question Sets------*/ /* Putting Quiz Questions in------*/ var defQuestions = [
    {
...
/*Progress Bar----------*/ const progressBar = document.getElementById('waterSupply-quizProgress');
let progress = progressBar.percentage;
/*Setting Progress Bar for Quiz------*/ function progressInitial() {
    quesIndex = 0;
    progressStart = 0;
...
/* Create Activator to let Answer Button be clickable on Answer selected.----*/ Array.from(quizSet).forEach(function(quizSetArray) {
    quizSetArray.addEventListener("click", function() {
        quizSet.checked = true;
        ansBtn.disabled = false;
    });
});
/* End Quiz Tasks--------*/ /*Render Dialogue box------*/ var dialog = document.getElementById('dialog-overview');
...
/*-------END Render Dialogue box-------*/ /*Create Activator to shift to next Question on click of button.-----*/ const maxPercent = 100;
